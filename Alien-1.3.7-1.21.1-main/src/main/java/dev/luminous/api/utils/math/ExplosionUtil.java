...
/*            if (player.hasStatusEffect(StatusEffects.RESISTANCE)) {
    ...
    public static int getProtectionAmount(Iterable<ItemStack> armorItems) {
        ...
        for (ItemStack itemStack : new ArrayList<>((DefaultedList<ItemStack>) armorItems)) {
            int level = InventoryUtil.getEnchantmentLevel(itemStack, Enchantments.PROTECTION);
            if (level == 0) {
                value += InventoryUtil.getEnchantmentLevel(itemStack, Enchantments.BLAST_PROTECTION) * 2;
            } else {
                value += level;
            }
        }
