        ...
        if (render.getValue()) {
            if (esp.getValue()) {
                ...
                for (double i = 0; i < 0.8; i += temp) {
                    HoleSnap.drawCircle(matrixStack, ColorUtil.injectAlpha(color.getValue(), (int) Math.min(color.getValue().getAlpha() * 2 / (0.8 / temp), 255)), i, new Vec3d(MathUtil.interpolate(mc.player.lastRenderX, mc.player.getX(), Render3DUtil.getTickDelta()), MathUtil.interpolate(mc.player.lastRenderY, mc.player.getY(), Render3DUtil.getTickDelta()), MathUtil.interpolate(mc.player.lastRenderZ, mc.player.getZ(), Render3DUtil.getTickDelta())), 5);
                }
