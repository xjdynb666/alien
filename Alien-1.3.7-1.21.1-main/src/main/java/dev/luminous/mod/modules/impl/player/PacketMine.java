...
public class PacketMine extends Module {
		...
		if (hotBar.getValue()) {
			...
			for (Map.Entry<Integer, ItemStack> entry : InventoryUtil.getInventoryAndHotbarSlots().entrySet()) {
				if (!(entry.getValue().getItem() instanceof AirBlockItem)) {
					final float digSpeed = InventoryUtil.getEnchantmentLevel(entry.getValue(), Enchantments.EFFICIENCY);
	...
	public float getDigSpeed(BlockState state, ItemStack itemStack) {
		...
		if (digSpeed > 1) {
			int efficiencyModifier = InventoryUtil.getEnchantmentLevel(itemStack, Enchantments.EFFICIENCY);
		...
		if (mc.player.isSubmergedInWater() && !(InventoryUtil.getEquipmentLevel(mc.player, Enchantments.AQUA_AFFINITY) > 0)) {
			digSpeed /= 5;
		}
