...
import dev.luminous.mod.Category;
import dev.luminous.api.settings.NumberSetting;
...
public class TargetHUD extends Mod {
    ...
    // 位置自定义
    public final NumberSetting xPos = new NumberSetting("X", 20, 0, 1920, 1);
    public final NumberSetting yPos = new NumberSetting("Y", 20, 0, 1080, 1);
    // 大小自定义
    public final NumberSetting width = new NumberSetting("Width", 180, 100, 300, 1);
    public final NumberSetting height = new NumberSetting("Height", 55, 30, 100, 1);
    // 样式自定义
    public final NumberSetting radius = new NumberSetting("Radius", 6, 0, 20, 1);
    public final NumberSetting alpha = new NumberSetting("Alpha", 220, 50, 255, 1);

    public TargetHUD() {
        super("TargetHUD", "显示攻击模块锁定的目标信息", Category.RENDER);
        addSettings(xPos, yPos, width, height, radius, alpha);
    }
    ...
    public void onRender3D(Render3DEvent event) {
        ...
        target = null;
        // 1. KillAura（优先）
        KillAura killAura = (KillAura) Alien.moduleManager.getModule(KillAura.class);
        if (killAura != null && killAura.isEnabled()) {
            // 兼容KillAura的getTarget()或target变量（通用写法）
            if (killAura instanceof LivingEntity) target = killAura;
            try {
        ...
        if (target == null) {
            AutoCrystal autoCrystal = (AutoCrystal) Alien.moduleManager.getModule(AutoCrystal.class);
            if (autoCrystal != null && autoCrystal.isEnabled() && autoCrystal.displayTarget != null && autoCrystal.displayTarget.isAlive()) {
        ...
        // 3. AutoAnchor（最后）
        if (target == null) {
            AutoAnchor autoAnchor = (AutoAnchor) Alien.moduleManager.getModule(AutoAnchor.class);
            if (autoAnchor != null && autoAnchor.isEnabled()) {
                // 兼容AutoAnchor的target/getTarget，和KillAura同逻辑
                try {
