...
public class TargetHUD extends Module {
    ...
    private void drawPlayerHead(MatrixStack matrices, PlayerEntity player, float x, float y, float w, float h) {
        ...
        // 调用1.21.1兼容的drawTexture方法
        mc.textRenderer.drawTexture(matrices, 0, 0, 8, 8, 8, 8, 8, 8, 64, 64);
        // 绘制头盔（如果有）
        if (player.canRenderCape()) {
            mc.textRenderer.drawTexture(matrices, 0, 0, 8, 8, 40, 8, 8, 8, 64, 64);
        }
        matrices.pop();
    }
