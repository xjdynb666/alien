		...
		if (item.booleanValue || player.booleanValue) {
			for (Entity entity : mc.world.getEntities()) {
				if (entity instanceof ItemEntity && item.booleanValue) {
					...
					Render3DUtil.draw3DBox(matrixStack, ((IEntity) entity).getDimensions().getBoxAt(new Vec3d(MathUtil.interpolate(entity.lastRenderX, entity.getX(), Render3DUtil.getTickDelta()), MathUtil.interpolate(entity.lastRenderY, entity.getY(), Render3DUtil.getTickDelta()), MathUtil.interpolate(entity.lastRenderZ, entity.getZ(), Render3DUtil.getTickDelta()))), color, false, true);
				} else if (entity instanceof PlayerEntity && player.booleanValue) {
					Color color = this.player.getValue();
					Render3DUtil.draw3DBox(matrixStack, ((IEntity) entity).getDimensions().getBoxAt(new Vec3d(MathUtil.interpolate(entity.lastRenderX, entity.getX(), Render3DUtil.getTickDelta()), MathUtil.interpolate(entity.lastRenderY, entity.getY(), Render3DUtil.getTickDelta()), MathUtil.interpolate(entity.lastRenderZ, entity.getZ(), Render3DUtil.getTickDelta()))).expand(0, 0.1, 0), color, false, true);
				}
			}
		}
