        ...
        for (Entity entity : mc.world.getEntities()) {
            if (entity instanceof EnderPearlEntity pearl) {
                ...
                if (ent.getMainHandStack().getItem() == Items.ENDER_PEARL || ent.getOffHandStack().getItem() == Items.ENDER_PEARL) {
                    double x = ent.prevX + (ent.getX() - ent.prevX) * Render3DUtil.getTickDelta();
                    double y = ent.prevY + (ent.getY() - ent.prevY) * Render3DUtil.getTickDelta();
                    double z = ent.prevZ + (ent.getZ() - ent.prevZ) * Render3DUtil.getTickDelta();
                    calcTrajectory(ent.getYaw(Render3DUtil.getTickDelta()), ent.getPitch(Render3DUtil.getTickDelta()), new Vec3d(x,y,z), ent.getVelocity());
                }
            }
        }
